(() => {
	function replaceNanaNuxtData() {
		localStorage.removeItem('__NANA_NUXT__');
		if (window.__NUXT__) {
			localStorage.setItem('__NANA_NUXT__', JSON.stringify(window.__NUXT__));
		}
	}

	function isSoundPage() {
		return window.location.pathname.includes('/sounds/');
	}

	async function insertDownloadButton() {
		const li = document.createElement('li');
		li.onclick = () => {
			const data = localStorage.getItem('__NANA_NUXT__');
			if (data) {
				const parsedData = JSON.parse(data);
				const soundUrl = parsedData.state.posts.mainPost.sound_url;
		...
		setTimeout(() => {
			const copyBtn = document.querySelector('.share-btn__list .copy-btn');
